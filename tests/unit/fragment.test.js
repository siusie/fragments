...
describe('Fragment class', () => {
  ...
  describe('save(), getData(), setData(), byId(), byUser(), delete()', () => {
    ...
    test('byUser() returns a non-empty array if this user has fragments', async () =>
    {
      const data = Buffer.from('hello');
      const fragment = new Fragment({ ownerId: '9000', type: 'text/plain', size: 5 });
      await fragment.save();
      await fragment.setData(data);
      expect((await Fragment.byUser(fragment.ownerId)).length).not.toBe(0);
    });

    test('create, save, and get a fragment\'s data', async () => {
      const data = Buffer.from('hello');
      const fragment = new Fragment({ ownerId: '1000', type: 'text/plain', size: 0 });
      await fragment.save();
      await fragment.setData(data);
      expect(await fragment.getData()).toEqual(data);

      const fragmentFromDB = await Fragment.byId(fragment.ownerId, fragment.id);
      expect(await fragmentFromDB.id).toEqual(fragment.id);

    });

    // TODO: ownerId is hashed email??? but argument passed to byId() is not???
    ...
    test('a fragment can be deleted', async () => {
      ...
      const ownerId = fragment.ownerId;
      await Fragment.delete(fragment.ownerId, fragment.id);
      expect(() => Fragment.byId(ownerId, id)).rejects.toThrow();
    });
  });
});
