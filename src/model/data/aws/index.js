...
// Delete a fragment's data from S3, metadata from memory db
async function deleteFragment(ownerId, id) {  
  ...
  try {
    return Promise.all([
      await ddbDocClient.send(commandDynamo),
      await s3Client.send(command),
    ]);
  } catch (err) {
    const { Bucket, Key } = params;
    const { TableName } = paramsDynamo; 
    logger.error({ err, Bucket, Key, TableName }, 'Error deleting fragment');
