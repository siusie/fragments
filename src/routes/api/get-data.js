...
module.exports = async (req, res) => { 
  ...
  // and won't extract id if extension's case and req.originalUrl's case do not match so convert both to lowercase
  ...
  try {
    const fragment = await Fragment.byId(req.user, id); // attempt to retrieve the fragment
    ...
    const data = type ? await fragment.convertData(type) : await fragment.getData();
